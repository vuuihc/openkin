...
import { assertActionAllowed, assertAllowedURL, isPrivateHost, requiresApproval, sanitizeActionForEvidence, } from "../src/policy.js";
const config = {
...
describe("browser policy", () => {
    it("allows exact and subdomains but rejects lookalikes", () => {
        expect(assertAllowedURL("https://example.com/a", config.allowed_domains).hostname).toBe("example.com");
        expect(assertAllowedURL("https://app.example.com/a", config.allowed_domains).hostname).toBe("app.example.com");
    ...
    it("requires approval for declared side effects", () => {
        const action = { type: "click", selector: "#send", side_effect: true };
    ...
    it("rejects runtime actions outside the typed protocol", () => {
        expect(() => assertActionAllowed({ type: "shell" }, config)).toThrow("unsupported browser action");
    });
